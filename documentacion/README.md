...
| `planes/mobile-grupos-horarios.md` | Plan vigente: creación de grupos y horarios (Fase 4, ítem 2) — formulario de grupo asociado a locación física (con alta mínima de locación anticipada de Fase 5.1), listado "Mis grupos" y detalle con asignación directa de alumnos (miembros `activo`) vía RPC `editar_miembros_grupo`; `codigo_invitacion` pasa a nullable, fuera de alcance v1 (implementado). |
