...
| `planes/mobile-fix-flicker-onboarding-login.md` | Plan vigente: fix del **parpadeo del onboarding al iniciar sesión** — `perfilResuelto`/`resolviendoPerfil` en `AuthGlobal` + gate `cargando || resolviendoPerfil` en `_layout`; el login pasa de "Cargando…" directo al Inicio (implementado). |
