...
- [x] **Panel del Organizador:** Desarrollar el panel alimentado exclusivamente por inscripciones confirmadas, donde el organizador tendrá control total para modificar las llaves manualmente si lo considera necesario.
  - `/panel/organizador` (listado de torneos propios + confirmados) y `/panel/organizador/[torneoId]` (detalle). Solo lectura de `estado='confirmado'` (filtro + RLS). Editor manual en `lib/llaves/` (TS puro: mover, liberar/bye, agregar sin ubicar; invariante de aparición única POR categoría → doble categoría, y poda de enfrentamientos vacíos) persistido atómicamente vía RPC `guardar_llaves_manuales` (mismo contrato jsonb que `generar_llaves`, valida `organizador_id` + `armado_llaves` + inscripciones confirmadas + sin repetidos dentro de una categoría + sin auto-enfrentamientos). Advertencias infantiles ≤5 kg y de rango/edad con `window.confirm`, no bloqueantes. `lib/emparejamiento/` intacto (53 tests Vitest OK). Detalles en `documentacion/planes/panel-organizador.md` y `documentacion/planes/doble-categoria.md`.
