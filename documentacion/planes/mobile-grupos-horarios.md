...
5. **Alta de locación con valor pactado (corrige: alineado al SRS §3.3):** `locaciones` registra `nombre`, `direccion` (obligatoria) y **`valor_alquiler` pactado (obligatorio, >= 0)**. El **monto pagado** de cada periodo sigue viviendo solo en `pagos_alquiler` (Fase 5.2). No confundir valor pactado (contrato) con monto pagado (transacción).
