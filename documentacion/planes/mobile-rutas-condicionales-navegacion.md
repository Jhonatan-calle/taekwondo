...
7. **Maestro ve Instructor (decisión):** pestaña "Instructor" visible si `esInstructor` (contexto); "Maestro" solo si `esMaestro`.
...
- JS Tabs de `expo-router`; ocultamiento condicional **sin envoltorios** vía `href` en `options` (nativo, sin romper la barra inferior):
  ...
  export default function TabsLayout() {
    ...
    return (
      <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: '#C62828', tabBarInactiveTintColor: '#666' }}>
        <Tabs.Screen name="index" options={{ title: 'Inicio' }} />
        <Tabs.Screen name="instructor" options={{ title: 'Instructor', href: esInstructor ? undefined : null }} />
...
- **Redirección contra deep links al inicio del renderizado:** si `!esInstructor`, renderizar `<Redirect href="/" />` (de `expo-router`); también pausar/evitar el resto del render.
...
- [ ] (Dispositivo) Profesor (`es_profesor` + Dan) → pestañas "Inicio" + "Instructor".
- [ ] (Dispositivo) Maestro sin `es_profesor` → "Inicio" + "Instructor" + "Maestro".
